    ...
    <>
      {props?.data && <Content data-component="Banner" background_image={props?.data?.field_imagem[0]?.url} className={`overflow-hidden ${props?.id}`}>
        <Container className="container d-flex flex-column">
          ...
          <Title dangerouslySetInnerHTML={{__html: props?.data?.field_title[0]?.value ? props?.data?.field_title[0]?.value  : ( 
            typeof props?.page?.title == 'object' ? props?.page?.title[0]?.value : `${props?.page?.title.substr(0, 60)}...`
          )}}></Title>
        </Container>
