...
export default function Breadcrumbs(props: any) {
  ...
  useEffect(() => {
    setData(
      props?.data?.page?.page ? props?.data?.page?.page : props?.data?.page
    );
  }, [props]);
  ...
  const handleBreadcrumbPaths = () => {
    ...
    Promise.all(
      ["/api/menu_items/main", "/api/menu_items/rodape"].map(function (
        url: any
      ) {
        return http.get(url);
      })
      ...
      .then((response: any) => {
        ...
        let group = nav.find(
          (link: any) => path.indexOf(slugify(link?.title).toLowerCase()) !== -1
        );

        if (data?.title[0]?.value !== group?.title) {
          paths.push(group?.title);

          let subgroup = group?.below.find(
            (link: any) =>
              path.indexOf(slugify(link?.title).toLowerCase()) !== -1
          );
        ...
        if (paths)
          setPaths(
            paths.filter(
              (link: any) => link !== "undefined" && link !== undefined
            )
          );
      })
      .catch(console.error);
  ...
  return (
    <Content className="breadcrumbs d-inline-flex align-items-center justify-content-center me-auto">
      ...
      {data &&
      props?.data?.page?.type &&
      typeof props?.data?.page?.type == "string" ? (
        <>
          <Link
            className="nav-link d-none d-md-flex"
            href={handleArchive(props?.data?.page?.type)}
          >
            <span>{props?.data?.page?.type}</span>
          </Link>
          <Link className="nav-link d-none d-md-flex" href="#">
            <span>
              {typeof data?.title == "object"
                ? data?.title[0]?.value
                : data?.title}
      ...
      {s && (
        <Link className="nav-link d-none d-md-flex" href="#">
          <span>{`Resultados de busca para "${s}"`}</span>
        </Link>
      )}
    </Content>
  );
